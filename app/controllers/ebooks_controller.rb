class EbooksController < ApplicationController
  ...
  def update
    ...
    old_input = params
    render(:edit, locals: { ebook: ebook, params: old_input }, status: :unprocessable_entity)
  end
  ...
  def send_users_email(ebook)
    users = User.all
    users.each do |user|
      UserMailer.with(user: user, ebook: ebook).changestatus_email.deliver_later unless user.admin?
    end
  end
end
